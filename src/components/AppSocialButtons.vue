<template>
  <div>
    <div class="social-links">
      ...
      <template v-for="(href, key) in $props"><!-- TODO v-if below is too complicated, simplify -->
        <div v-if="href && key !== 'buttonSize' && key !== 'buttonSizeUnit'" :key="'about__social_link_' + key" class="social-links__link-container">
          <a :href="href">
            <img :src="getImgUrl(icons[key])" :alt="key" class="social-links__link" :style="buttonStyle">
          </a>
        </div>
      </template>
    </div>
...
<script>
  export default {
    ...
    props: { //TODO strict type check for all props everywhere
      vk: String,
      in: String,
      yt: String,
      buttonSize: Number,
